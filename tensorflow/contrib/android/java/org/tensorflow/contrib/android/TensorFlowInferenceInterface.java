...
import org.tensorflow.contrib.android.cipher.CReaderImpl;
...
public class TensorFlowInferenceInterface {
    ...
    public TensorFlowInferenceInterface(AssetManager assetManager, String model) {
        ...
        try {
            ...
            CReaderImpl.load(graphDef);

            loadGraph(graphDef, g);
            is.close();
            TFLog(TAG, "Successfully loaded model from '" + model + "'");
    ...
    public TensorFlowInferenceInterface(InputStream is) {
        ...
        try {
            ...
            loadGraph(graphDef, g);
            TFLog(TAG, "Successfully loaded model from the input stream");
    ...
    private void prepareNativeRuntime() {
        TFLog(TAG, "Checking to see if TensorFlow native methods are already loaded");
        try {
            // Hack to see if the native libraries have been loaded.
            new RunStats();
            TFLog(TAG, "TensorFlow native methods already loaded");
        } catch (UnsatisfiedLinkError e1) {
            TFLog(
                    TAG, "TensorFlow native methods not found, attempting to load via tensorflow_inference");
            try {
                System.loadLibrary("tensorflow_inference");
                TFLog(TAG, "Successfully loaded TensorFlow native methods (RunStats error may be ignored)");
    ...
    private void TFLog(String tag, String message) {
